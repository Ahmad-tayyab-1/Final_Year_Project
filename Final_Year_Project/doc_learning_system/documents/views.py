...
# pyrefly: ignore [missing-import]
from .utils import DocumentProcessor, AIAssistant, YouTubeProcessor
...
def home(request):
    if request.user.is_authenticated:
        documents = Document.objects.filter(user=request.user).prefetch_related("chat_messages", "flashcards").all()
    else:
        ids = _guest_doc_ids(request)
        documents = Document.objects.filter(id__in=ids, user__isnull=True).prefetch_related("chat_messages", "flashcards") if ids else []
    guest_remaining = max(0, GUEST_UPLOAD_LIMIT - len(_guest_doc_ids(request))) if not request.user.is_authenticated else None
...
def upload_document(request):
    if not request.user.is_authenticated and not _guest_can_upload(request):
        messages.info(request, "You've used your 3 free uploads. Please create a free account to continue.")
    ...
    doc = Document.objects.create(user=request.user if request.user.is_authenticated else None, title=title, file=file, file_type=file_ext)
    try:
        proc = DocumentProcessor()
        if file_ext == "pdf":
            text, pages, total_pages = proc.extract_text_with_pages(doc.file.path)
            doc.total_pages = total_pages
        elif file_ext in ["doc", "docx", "pptx", "xlsx"]:
            # Extract text for AI processing
            try:
                if file_ext in ["doc", "docx"]:
                    text = proc.extract_text_from_doc(doc.file.path) if file_ext == "doc" else proc.extract_text_from_docx(doc.file.path)
                elif file_ext == "pptx":
                ...
                messages.warning(request, f"Could not fully extract text from file: {extract_err}. File will still be processed.")
...
def process_youtube(request):
    ...
    if not video_id:
        messages.error(request, "Could not parse a video ID from that URL. Please use a standard YouTube link.")
        return redirect("documents:upload")
...
def chat(request, doc_id):
    ...
    ai = AIAssistant()
    bot_response, page_ref = ai.answer_question(user_message, doc.text_content)
...
def create_mcqs_from_text(request, doc_id):
    ...
    for m in AIAssistant().generate_mcqs(text, count):
        mcq = MCQQuestion.objects.create(
            ...
            correct_answer=m["correct_answer"], explanation=m.get("explanation",""),
            source_text=text[:500], page_number=page_number,
        )
        created.append({
...
def create_short_questions_from_text(request, doc_id):
    ...
    for q in AIAssistant().generate_short_questions(text, count):
        ...
        created.append({"id": sq.id, "question": sq.question, "answer": sq.answer})
